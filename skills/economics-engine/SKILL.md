...
3. Calculate:
   - **Daily burn rate**: Total costs in window / number of days
   - **Monthly projected burn**: Daily burn * 30
   - **Runway**: If revenue > 0: (monthly budget tolerance - monthly projected burn) / daily burn rate = days until budget exhausted. If revenue = 0: daily budget limit / daily burn rate = days of budget remaining
4. Return:
...
- `paused`: utilization >= 100% and < 300% — non-critical agents should pause
- `kill-switch`: utilization >= 300% — ALL non-essential agents must stop immediately
...
3. Calculate:
   ...
   - `revenueToExpenseRatio = totalRevenue / totalCosts` (0 if no costs)
   - `netMargin = (totalRevenue - totalCosts) / totalRevenue * 100` (0 if no revenue)
...
| **300% (kill-switch)** | ALL agents except orchestrator stop immediately. Orchestrator sends emergency alert to founder. No new API calls except the alert itself. |

### Post-Kill-Switch Recovery

1. Orchestrator sends founder a message: "Daily budget exceeded 3x. All agents paused. Current spend: $X / $Y limit."
