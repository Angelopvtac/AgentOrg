...
- `F2.1-T7` — Implement budget enforcement: daily budget tracking, per-agent allocation, kill switch at 2x threshold
...
| F7.1-S5 | As the orchestrator, I enforce daily budgets and trigger kill switch at 2x overspend | P0 | done |
...
- `F7.1-T10` — Implement daily budget enforcement: track spend per day, per agent, alert at 80%, pause non-critical at 100%, kill switch at 2x
...
- `F16.2-T1` — Implement kill switch in economics engine: detect 2x overspend → pause agents → alert founder
