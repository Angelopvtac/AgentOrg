...
with open(sys.argv[1]) as f:
    text = f.read()
# Strip single-line comments — only outside quoted strings
# Match // not preceded by : (avoids destroying URLs like https://...)
...
if [ -f "$EFILE" ]; then
    ...
    check_field "$EFILE" "d['alerts']['killSwitchAt'] == 2.0" "economics.json kill switch at 2x"
fi
